...
public class StringUtil {
	...
	/* Borrowed utilities from Avantrade */
	...
	public static String formatNumber(String num, int precision,
			int minPrecision, boolean useGrouping) {
		try {
	...
	public static String reFormat(String number) {
		...
		try {
			...
			answer = (new java.math.BigDecimal(a.parse(number).toString()))
					.toString();
		} catch (Exception e) {
			e.getMessage();
		}
	...
	public static String nullStr(String parameter, String defaultString) {
		return (parameter == null || parameter.equalsIgnoreCase("") || parameter
				.equalsIgnoreCase("null")) ? defaultString : parameter;
	}
	...
	public static String padZeros(String str, int len) {
		return (str.length() > len) ? str.substring(0, len) : pad(str, len,
				'0', StringUtil.LEFT);
	}
	...
	public static String extractFilename(String file) {
		...
		int lastIndex = (file.lastIndexOf("\\") > file.lastIndexOf("/")) ? file
			...
			if (spaceIdx > 0) {
				...
				if ((spaceIdxA >= 0) & (spaceIdxB == -1)) {
					return rTrim(str.substring(0, spaceIdxA))
							+ NEW_LINE
							+ lineWrap(str.substring(spaceIdxA).trim(),
									borderLine);
				...
				// space found after borderline
					...
					return rTrim(str.substring(0, (borderLine + spaceIdxB)))
							+ NEW_LINE
							+ lineWrap(str.substring((borderLine + spaceIdxB))
									.trim(), borderLine);
					...
					if (diff <= spaceIdxB)
						...
						return rTrim(str.substring(0, (borderLine + spaceIdxB)))
								...
								+ lineWrap(str.substring(
										(borderLine + spaceIdxB)).trim(),
										borderLine);

					else
						return rTrim(str.substring(0, borderLine - 3))
								+ NEW_LINE
								+ lineWrap(str.substring((borderLine - 3))
										.trim(), borderLine);
				} else
					return rTrim(str.substring(0, borderLine - 3))
							+ NEW_LINE
							+ lineWrap(str.substring((borderLine - 3)).trim(),
									borderLine);
	...
	public static String[] subString(String string, int numOfChar,
			int numOfString) {
